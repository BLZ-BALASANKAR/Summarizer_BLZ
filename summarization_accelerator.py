...
#                   abstractive_min_length: int = 50) -> Dict:
#         """
...
#                     text, 
#                     sentence_count=extractive_sentence_count
#                 )
#                 results['extractive_summaries'] = extractive_results
#                 print(f"{Fore.GREEN}✓ Completed {len(extractive_results)} extractive algorithms{Style.RESET_ALL}")
#             except Exception as e:
#                 print(f"{Fore.RED}✗ Error in extractive summarization: {str(e)}{Style.RESET_ALL}")
...
#                             abstractive_results['pegasus_xsum'] = self.abstractive.pegasus_summarize(
#                                 text, abstractive_max_length, abstractive_min_length
#                             )

#                         elif model_key == 'flan_t5_base':
#                             abstractive_results['flan_t5_base'] = self.abstractive.flan_t5_summarize(
#                                 text, abstractive_max_length, abstractive_min_length, variant="base"
#                             )

#                         elif model_key == 'flan_t5_large':
#                             abstractive_results['flan_t5_large'] = self.abstractive.flan_t5_summarize(
#                                 text, abstractive_max_length, abstractive_min_length, variant="large"
#                             )

#                         elif model_key == 'pegasus_large':
#                             abstractive_results['pegasus_large'] = self.abstractive.pegasus_large_summarize(
#                                 text, abstractive_max_length, abstractive_min_length
#                             )

#                         elif model_key == 'bigbird_pegasus':
#                             abstractive_results['bigbird_pegasus'] = self.abstractive.bigbird_pegasus_summarize(
#                                 text, abstractive_max_length, abstractive_min_length
#                             )

#                         elif model_key == 'led':
#                             abstractive_results['led'] = self.abstractive.led_summarize(
#                                 text, max_length=abstractive_max_length, min_length=abstractive_min_length
#                             )
...
#                 print(f" Completed {len(abstractive_results)} abstractive models{Style.RESET_ALL}")

#             except Exception as e:
#                 print(f" Error in abstractive summarization: {str(e)}{Style.RESET_ALL}")
...
#         # Header
#         print("\n" + "╔" + "═"*118 + "╗")
#         print("║" + Fore.CYAN + Style.BRIGHT + " "*45 + "� SUMMARIZATION RESULTS" + " "*49 + Style.RESET_ALL + "║")
#         print("╚" + "═"*118 + "╝\n")
...
#             print(Fore.YELLOW + f" Original Text: {len(words)} words | {len(original)} characters" + Style.RESET_ALL + "\n")
        
#         # Print extractive summaries in table format
...
#                   Fore.CYAN + Style.BRIGHT + " Method".ljust(20) + Style.RESET_ALL + "│" + 
#                   Fore.CYAN + Style.BRIGHT + " Summary".ljust(70) + Style.RESET_ALL + "│" + 
#                   Fore.CYAN + Style.BRIGHT + " Words  " + Style.RESET_ALL + "│" + 
#                   Fore.CYAN + Style.BRIGHT + " Time (s)   " + Style.RESET_ALL + "│")
#             print("├" + "─"*3 + "┼" + "─"*20 + "┼" + "─"*70 + "┼" + "─"*8 + "┼" + "─"*12 + "┤")
            
#             for idx, (algo, data) in enumerate(results['extractive_summaries'].items(), 1):
#                 if 'error' in data:
#                     print(f"│ {idx} │ {algo[:18].ljust(20)} │ {Fore.RED}ERROR: {data['error'][:60]}{Style.RESET_ALL}".ljust(70) + " │        │            │")
#                 else:
#                     summary = data['summary']
#                     words_list = summary.split()
#                     summary_preview = summary[:66] + "..." if len(summary) > 66 else summary
                    
#                     # First row with preview
#                     print(f"│ {idx} │ {algo.replace('_', ' ').title()[:18].ljust(20)} │ {summary_preview[:68].ljust(70)} │ {len(words_list):>6} │ {data.get('execution_time', 0):>10.3f} │")
                    
#                     # Additional rows for longer summaries
#                     remaining = summary[66:]
#                     while remaining:
#                         chunk = remaining[:68]
#                         remaining = remaining[68:]
#                         print(f"│   │{' '*20}│ {chunk.ljust(70)} │        │            │")
            
#             print("└" + "─"*3 + "┴" + "─"*20 + "┴" + "─"*70 + "┴" + "─"*8 + "┴" + "─"*12 + "┘\n")
        
#         # Print abstractive summaries in table format
#         if results.get('abstractive_summaries'):
#             print(Fore.MAGENTA + Style.BRIGHT + " ABSTRACTIVE SUMMARIES" + Style.RESET_ALL)
#             print("┌" + "─"*3 + "┬" + "─"*20 + "┬" + "─"*70 + "┬" + "─"*8 + "┬" + "─"*12 + "┐")
#             print("│" + Fore.CYAN + Style.BRIGHT + " # " + Style.RESET_ALL + "│" + 
#                   Fore.CYAN + Style.BRIGHT + " Model".ljust(20) + Style.RESET_ALL + "│" + 
#                   Fore.CYAN + Style.BRIGHT + " Summary".ljust(70) + Style.RESET_ALL + "│" + 
#                   Fore.CYAN + Style.BRIGHT + " Words  " + Style.RESET_ALL + "│" + 
#                   Fore.CYAN + Style.BRIGHT + " Time (s)   " + Style.RESET_ALL + "│")
#             print("├" + "─"*3 + "┼" + "─"*20 + "┼" + "─"*70 + "┼" + "─"*8 + "┼" + "─"*12 + "┤")
            
#             for idx, (model, data) in enumerate(results['abstractive_summaries'].items(), 1):
#                 if 'error' in data:
#                     print(f"│ {idx} │ {model[:18].ljust(20)} │ {Fore.RED}ERROR: {data['error'][:60]}{Style.RESET_ALL}".ljust(70) + " │        │            │")
#                 else:
#                     summary = data['summary']
#                     words_list = summary.split()
#                     summary_preview = summary[:66] + "..." if len(summary) > 66 else summary
...
#                     # First row with preview
#                     print(f"│ {idx} │ {model.replace('_', ' ').title()[:18].ljust(20)} │ {summary_preview[:68].ljust(70)} │ {len(words_list):>6} │ {data.get('execution_time', 0):>10.3f} │")
...
#         # Print metadata summary in table format
#         if verbose and results.get('metadata'):
#             meta = results['metadata']
#             print(Fore.YELLOW + Style.BRIGHT + "ℹ  EXECUTION SUMMARY" + Style.RESET_ALL)
#             print("┌" + "─"*35 + "┬" + "─"*20 + "┐")
#             print(f"│ Total Execution Time            │ {Fore.GREEN}{meta['total_execution_time']:>16.2f}s{Style.RESET_ALL} │")
#             print(f"│ Extractive Models Used          │ {Fore.GREEN}{meta['extractive_count']:>19}{Style.RESET_ALL} │")
#             print(f"│ Abstractive Models Used         │ {Fore.GREEN}{meta['abstractive_count']:>19}{Style.RESET_ALL} │")
#             print(f"│ Timestamp                       │ {meta['timestamp']:>20} │")
#             print("└" + "─"*35 + "┴" + "─"*20 + "┘\n")
...
#     def save_to_excel(self, results: Dict, evaluation_results: Dict = None, filename: str = "summarization_results.xlsx"):
#         """
#         Save results and evaluations to an Excel file with proper formatting
        
#         Args:
#             results: Results from summarize() method
#             evaluation_results: Results from evaluate_results() method (optional)
...
#                         'Execution Time (s)': round(data.get('execution_time', 0), 3),
#                     }
                    
#                     # Add evaluation metrics if available
#                     if evaluation_results and evaluation_results.get('extractive_evaluations'):
#                         metrics = evaluation_results['extractive_evaluations'].get(algo, {})
#                         row['Compression Ratio (%)'] = round(metrics.get('compression_ratio', 0) * 100, 2)
#                         row['Density (%)'] = round(metrics.get('density', 0) * 100, 2)
#                         row['Cosine Similarity'] = round(metrics.get('text_similarity', 0), 4)
                        
#                         rouge = metrics.get('rouge', {})
#                         if 'error' not in rouge:
#                             row['ROUGE-1 F1'] = round(rouge.get('rouge1_f', 0), 4)
#                             row['ROUGE-2 F1'] = round(rouge.get('rouge2_f', 0), 4)
#                             row['ROUGE-L F1'] = round(rouge.get('rougeL_f', 0), 4)
                        
#                         bleu = metrics.get('bleu', {})
#                         if 'error' not in bleu:
#                             row['BLEU Score'] = round(bleu.get('bleu_overall', 0), 4)
                        
#                         meteor = metrics.get('meteor', {})
#                         if 'error' not in meteor:
#                             row['METEOR Score'] = round(meteor.get('meteor', 0), 4)
                    
#                     extractive_data.append(row)
            
#             if extractive_data:
#                 df_extractive = pd.DataFrame(extractive_data)
#                 df_extractive.to_excel(writer, sheet_name='Extractive Summaries', index=False)
...
#                     # Add evaluation metrics if available
#                     if evaluation_results and evaluation_results.get('abstractive_evaluations'):
#                         metrics = evaluation_results['abstractive_evaluations'].get(model, {})
#                         row['Compression Ratio (%)'] = round(metrics.get('compression_ratio', 0) * 100, 2)
#                         row['Density (%)'] = round(metrics.get('density', 0) * 100, 2)
#                         row['Cosine Similarity'] = round(metrics.get('text_similarity', 0), 4)
...
#                 df_abstractive.to_excel(writer, sheet_name='Abstractive Summaries', index=False)
...
#                     if evaluation_results and evaluation_results.get('extractive_evaluations'):
#                         metrics = evaluation_results['extractive_evaluations'].get(algo, {})
#                         row['Compression %'] = round(metrics.get('compression_ratio', 0) * 100, 2)
#                         row['Density %'] = round(metrics.get('density', 0) * 100, 2)
#                         row['Cosine Sim'] = round(metrics.get('text_similarity', 0), 4)
#                     all_data.append(row)
...
#             df_combined.to_excel(writer, sheet_name='All Results', index=False)
...
#             meta = results['metadata']
#             metadata_data = [
#                 {'Metric': 'Total Execution Time (s)', 'Value': round(meta.get('total_execution_time', 0), 2)},
#                 {'Metric': 'Extractive Models Used', 'Value': meta.get('extractive_count', 0)},
#                 {'Metric': 'Abstractive Models Used', 'Value': meta.get('abstractive_count', 0)},
#                 {'Metric': 'Timestamp', 'Value': meta.get('timestamp', '')},
#             ]
            
#             if results.get('input_text'):
#                 metadata_data.insert(0, {'Metric': 'Original Word Count', 'Value': len(results['input_text'].split())})
#                 metadata_data.insert(1, {'Metric': 'Original Character Count', 'Value': len(results['input_text'])})
...
#                 if column_letter == 'C' or 'Summary' in str(ws[f'{column_letter}1'].value):
#                     adjusted_width = 80  # Wider for summary column
...
#         from colorama import Fore, Style
#         print(f"{Fore.GREEN} Results saved to Excel file: {filename}{Style.RESET_ALL}")
#         print(f"{Fore.CYAN} Sheets created: {', '.join(wb.sheetnames)}{Style.RESET_ALL}")
...
#         Returns:
#             Dictionary with evaluation metrics for each summary
#         """
...
#         """
#         Print evaluation report in tabular format
        
#         Args:
#             evaluation_results: Results from evaluate_results() method
#             verbose: Whether to print detailed information
#         """
#         from colorama import Fore, Style
...
#         # Header
#         print("\n" + "╔" + "═"*118 + "╗")
#         print("║" + Fore.CYAN + Style.BRIGHT + " "*48 + " EVALUATION REPORT" + " "*50 + Style.RESET_ALL + "║")
#         print("╚" + "═"*118 + "╝\n")
        
#         # Extractive evaluations
#         if evaluation_results.get('extractive_evaluations'):
#             print(Fore.GREEN + Style.BRIGHT + " EXTRACTIVE SUMMARIES EVALUATION" + Style.RESET_ALL)
            
#             if verbose:
#                 # Detailed table with all metrics
#                 print("┌" + "─"*3 + "┬" + "─"*18 + "┬" + "─"*8 + "┬" + "─"*12 + "┬" + "─"*10 + "┬" + "─"*10 + "┬" + "─"*22 + "┬" + "─"*22 + "┬" + "─"*10 + "┐")
#                 print("│" + Fore.CYAN + " # " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Method".ljust(18) + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Words  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Compress   " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Density  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Cosine   " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " ROUGE-1 (P/R/F1)     " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " ROUGE-2 (P/R/F1)     " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " BLEU     " + Style.RESET_ALL + "│")
#                 print("├" + "─"*3 + "┼" + "─"*18 + "┼" + "─"*8 + "┼" + "─"*12 + "┼" + "─"*10 + "┼" + "─"*10 + "┼" + "─"*22 + "┼" + "─"*22 + "┼" + "─"*10 + "┤")
                
#                 for idx, (algo, metrics) in enumerate(evaluation_results['extractive_evaluations'].items(), 1):
#                     rouge = metrics.get('rouge', {})
#                     bleu = metrics.get('bleu', {})
                    
#                     rouge1_str = f"{rouge.get('rouge1_p', 0):.2f}/{rouge.get('rouge1_r', 0):.2f}/{Fore.GREEN}{rouge.get('rouge1_f', 0):.2f}{Style.RESET_ALL}" if 'error' not in rouge else "N/A"
#                     rouge2_str = f"{rouge.get('rouge2_p', 0):.2f}/{rouge.get('rouge2_r', 0):.2f}/{Fore.GREEN}{rouge.get('rouge2_f', 0):.2f}{Style.RESET_ALL}" if 'error' not in rouge else "N/A"
#                     bleu_str = f"{Fore.GREEN}{bleu.get('bleu_overall', 0):.4f}{Style.RESET_ALL}" if 'error' not in bleu else "N/A"
                    
#                     print(f"│ {idx} │ {algo.replace('_', ' ').title()[:16].ljust(18)} │ {metrics.get('generated_summary_length', 0):>6} │ {metrics.get('compression_ratio', 0):>10.2%} │ {metrics.get('density', 0):>8.2%} │ {metrics.get('text_similarity', 0):>8.4f} │ {rouge1_str:>22} │ {rouge2_str:>22} │ {bleu_str:>10} │")
                
#                 print("└" + "─"*3 + "┴" + "─"*18 + "┴" + "─"*8 + "┴" + "─"*12 + "┴" + "─"*10 + "┴" + "─"*10 + "┴" + "─"*22 + "┴" + "─"*22 + "┴" + "─"*10 + "┘\n")
#             else:
#                 # Compact table
#                 print("┌" + "─"*3 + "┬" + "─"*20 + "┬" + "─"*8 + "┬" + "─"*12 + "┬" + "─"*10 + "┬" + "─"*10 + "┐")
#                 print("│" + Fore.CYAN + " # " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Method".ljust(20) + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Words  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Compress   " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Density  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Cosine   " + Style.RESET_ALL + "│")
...
#                 for idx, (algo, metrics) in enumerate(evaluation_results['extractive_evaluations'].items(), 1):
#                     print(f"│ {idx} │ {algo.replace('_', ' ').title()[:18].ljust(20)} │ {metrics.get('generated_summary_length', 0):>6} │ {metrics.get('compression_ratio', 0):>10.2%} │ {metrics.get('density', 0):>8.2%} │ {metrics.get('text_similarity', 0):>8.4f} │")
                
#                 print("└" + "─"*3 + "┴" + "─"*20 + "┴" + "─"*8 + "┴" + "─"*12 + "┴" + "─"*10 + "┴" + "─"*10 + "┘\n")
        
#         # Abstractive evaluations
#         if evaluation_results.get('abstractive_evaluations'):
#             print(Fore.MAGENTA + Style.BRIGHT + " ABSTRACTIVE SUMMARIES EVALUATION" + Style.RESET_ALL)
            
#             if verbose:
#                 # Detailed table
#                 print("┌" + "─"*3 + "┬" + "─"*18 + "┬" + "─"*8 + "┬" + "─"*12 + "┬" + "─"*10 + "┬" + "─"*10 + "┬" + "─"*22 + "┬" + "─"*22 + "┬" + "─"*10 + "┐")
#                 print("│" + Fore.CYAN + " # " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Model".ljust(18) + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Words  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Compress   " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Density  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Cosine   " + Style.RESET_ALL + "│" + 
...
#                 for idx, (model, metrics) in enumerate(evaluation_results['abstractive_evaluations'].items(), 1):
#                     rouge = metrics.get('rouge', {})
#                     bleu = metrics.get('bleu', {})
...
#                     print(f"│ {idx} │ {model.replace('_', ' ').title()[:16].ljust(18)} │ {metrics.get('generated_summary_length', 0):>6} │ {metrics.get('compression_ratio', 0):>10.2%} │ {metrics.get('density', 0):>8.2%} │ {metrics.get('text_similarity', 0):>8.4f} │ {rouge1_str:>22} │ {rouge2_str:>22} │ {bleu_str:>10} │")
...
#                 print("┌" + "─"*3 + "┬" + "─"*20 + "┬" + "─"*8 + "┬" + "─"*12 + "┬" + "─"*10 + "┬" + "─"*10 + "┐")
#                 print("│" + Fore.CYAN + " # " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Model".ljust(20) + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Words  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Compress   " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Density  " + Style.RESET_ALL + "│" + 
#                       Fore.CYAN + " Cosine   " + Style.RESET_ALL + "│")
...
#                 for idx, (model, metrics) in enumerate(evaluation_results['abstractive_evaluations'].items(), 1):
#                     print(f"│ {idx} │ {model.replace('_', ' ').title()[:18].ljust(20)} │ {metrics.get('generated_summary_length', 0):>6} │ {metrics.get('compression_ratio', 0):>10.2%} │ {metrics.get('density', 0):>8.2%} │ {metrics.get('text_similarity', 0):>8.4f} │")
...
class SummarizationAccelerator:
    ...
    def save_to_excel(
        ...
        # 🔒 Force a fixed filename (ignore any caller override)
                ...
                if "error" not in data:
                    ...
                    if (
                        ...
                        row["Compression Ratio (%)"] = round(
                            metrics.get("compression_ratio", 0) * 100, 2
                        )
                        row["Density (%)"] = round(
                            metrics.get("density", 0) * 100, 2
                        )
                        row["Cosine Similarity"] = round(
                            metrics.get("text_similarity", 0), 4
                        )

                        rouge = metrics.get("rouge", {})
                        if "error" not in rouge:
                            row["ROUGE-1 F1"] = round(
                                rouge.get("rouge1_f", 0), 4
                            )
                            row["ROUGE-2 F1"] = round(
                                rouge.get("rouge2_f", 0), 4
                            )
                            row["ROUGE-L F1"] = round(
                                rouge.get("rougeL_f", 0), 4
                            )

                        bleu = metrics.get("bleu", {})
                        if "error" not in bleu:
                            row["BLEU Score"] = round(
                                bleu.get("bleu_overall", 0), 4
                            )

                        meteor = metrics.get("meteor", {})
                        if "error" not in meteor:
                            row["METEOR Score"] = round(
                                meteor.get("meteor", 0), 4
                            )
            ...
            if verbose:
                ...
                for idx, (algo, metrics) in enumerate(
                    ...
                    print(
                        ...
                        f"{metrics.get('compression_ratio', 0):>10.2%} | "
                        f"{metrics.get('density', 0):>8.2%} | "
                        f"{metrics.get('text_similarity', 0):>8.4f} | "
